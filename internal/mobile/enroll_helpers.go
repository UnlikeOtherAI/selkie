...
// 4, or 8 and breaks the hub's WireGuard semantics.
//
// These are the post-mask canonical low-order points; the validator masks the
// high bit (byte 31 &= 0x7f) before lookup per RFC 7748 §5, which says
// X25519 implementations ignore the high bit of the u-coordinate. Masking
// first means we only need one entry per point — the six former high-bit-set
// variants are fully covered by the mask. Sourced from the libsodium
